...
COMPONENT_VERSION="${VERSION:-"latest"}"
COMPONENT_SHA256="${SHA256:-"automatic"}"
...
install_from_git_release() {
    ...
    # Install component, verify signature and cCOMPONENT_VERSIONhecksum
    ...
    local component_filename="${component}_${requested_version}_linux_${architecture}"
    ...
    if [ "$requested_sha256" = "automatic" ]; then
        curl -sSL "https://github.com/${github_repo}/releases/download/v${requested_version}/${component}_${requested_version}_SHA256SUMS" -o /tmp/${component}/checksums
...
install_from_git_release terraform-config-inspect COMPONENT_VERSION COMPONENT_SHA256 neermitt/terraform-config-inspect
