...
install_from_git_release() {
    ...
    if [ "$requested_sha256" = "automatic" ]; then
        ...
        grep -m 1 "${component_filename}" /tmp/${component}/checksums

        requested_sha256="$(grep -m 1 "${component_filename}" /tmp/${component}/checksums | awk '{print $1}' )"
        echo "SHA256: ${requested_sha256}"
    fi
    ([ "${requested_sha256}" = "dev-mode" ] || (echo "${requested_sha256} */usr/local/bin/${component}" | sha256sum -c -))
...
install_from_git_release gomplate GOMPLATE_VERSION GOMPLATE_SHA256 "hairyhenderson/gomplate"
