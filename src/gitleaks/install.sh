...
GITLEAKS_SHA256="${SHA256:-"automatic"}"
...
install_from_git_release() {
    ...
    local component_filename="${component}_${requested_version}_linux_${architecture}.tar.gz"
    ...
    if [ "$requested_sha256" = "automatic" ]; then
        curl -sSL "https://github.com/${github_repo}/releases/download/v${requested_version}/${component}_${requested_version}_checksums.txt" -o /tmp/${component}/checksums
...
install_from_git_release gitleaks GITLEAKS_VERSION GITLEAKS_SHA256 "zricethezav/gitleaks"
